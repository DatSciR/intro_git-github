...
2.  Crea una carpeta llamada "datos" en tu directorio de trabajo. Añadela al *.gitignore* y guarda. ¿Qué ha pasado en la pestaña Git?
...
The history of a repository (*🕘XX commits*) contains a list of links to all commits made in any of the branches. Within each commit, you can see the files added or deleted in that "snapshot" and the lines of code added (in green) or deleted (in red) in each modified file. Additionally, in the history, you can add comments on specific lines of code or general comments on the entire commit.

On GitHub, you can also access the commit history of each specific file (*History*) and the author of each part of the code (*Blame*).

The project and file history is also accessible through RStudio (🕒).

💡More information about how to navigate the project’s past here: <https://happygitwithr.com/time-travel-see-past.html>

![Repository history](images/hist.jpg)

💡In *issues*, you can reference a specific line of code from a specific state of the project by entering the file in the *Code* tab and clicking on the line number you want to comment on, or reference an entire *commit* by copying the SHA.

💡If I want to go back in time or if I make a change I don’t want, how can I fix it? There are multiple options, but [here](https://github.com/DatSciR/intro_git-github/blob/main/centra/dia3_comandos.md) (in the section "I messed up, how do I undo changes?") we detail three: *restore*, *reset*, and *revert*.
