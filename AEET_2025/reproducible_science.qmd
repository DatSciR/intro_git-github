...
### 📝**Exersice 1.1**
At this point, you need to have the latest version of R installed (<https://cloud.r-project.org/>), RStudio (<https://www.rstudio.com/products/rstudio/download/>), Git (<https://happygitwithr.com/install-git.html>) y una cuenta en GitHub (<https://github.com/>) creada.
...
1.  Create a repository on GitHub and connect it to a new RStudio project (this will generate a repository (folder) on your computer in the location you specified). Include a "*.gitignore*" file.
Create a new R script in the working directory (that is, create an R script and save it inside the repository you just created)
In RStudio, go to the Git tab to see all the documents that have been identified by Git
...
GitHub is not just a remote repository to store different versions of your work or to develop collaborative projects — it’s also a meeting network for programmers. Like other social networks, you can browse profiles, follow people, have followers, and bookmark projects you like.

Using the search bar (🔍) you can look for content you're interested in. The search is organized by categories (*Repositories, Commits, Issues, Users*, etc.) making it easier to find what you need. To follow a user, click *Follow*. By clicking *Star* ⭐ you can bookmark a repository to your GitHub account, and with *Fork* you make a copy you can interact with. *Watch* 👁️ lets you track updates on a repository. *Download* allows you to save a copy of any public repository to your computer.

### Workflow in Git and GitHub


Git can track all the files in a repository. To understand how Git logs changes and how we can share those changes with collaborators, it’s important to grasp how Git is structured and how it syncs with GitHub. There are four "working areas":

1.  **Working directory:** where you're actively working. This area syncs with the files on your computer.

2.  **Staging area (or *Index*):** an intermediate area between the working directory and the local repository. It’s a draft zone where the user selects which files to record in the next "snapshot."

3.  **Local repository (or *HEAD*):** where all committed changes are recorded on your computer.

4.  **Remote repository:** where changes committed in Git are uploaded to the cloud (GitHub).

![Graphical representation of Git’s working areas. Image background by Philip Brookes (https://creativecommons.org/licenses/by-nc-nd/2.0/legalcode)](images/arboles.jpg)

### How to move from one area to another?

You can do this using the terminal command line or the Git tab in RStudio — the process is the same.

![Git options in RStudio](images/git_RStudio.png){fig-align="center" width="486"}

At first, all changes appear in yellow because Git doesn’t yet know what to do with them. We’re in the working directory, and not all changes need to be saved for the future.

To add a change from the working directory to the staging area, use `git add` (in RStudio’s Git tab you select the file). This command tells Git which file updates you want to include in the next project snapshot. `git add` does not affect the local repository.

-   `git add <filename>`: adds a file update from the working directory to the staging area.

To record the changes we want, we use `git commit` (in RStudio’s Git tab, by clicking the *Commit* button). Executing `git commit` makes a snapshot of the project’s state. Along with the commit, you add a short message explaining what and why changes were made (e.g. *"add formatted references"*). Each `git commit` has a unique SHA (Secure Hash Algorithm) — an alphanumeric code identifying the commit (e.g. `1d21fc3c33cxxc4aeb7823400b9c7c6bc2802be1`). Don’t worry about memorizing it — usually, the first seven digits are enough.

-   `git commit -m "short and descriptive message"`

![Commit in RStudio](images/commit_RStudio.png){fig-align="center" width="545"}

💡 Using `git commit` in your project is like placing anchors while climbing a rock face. Developing a script without commits is like climbing without protection: you might move faster in the short term, but the chances of a catastrophic failure increase. On the other hand, making too many commits slows your progress. The best practice: make more commits when you're navigating tricky or risky code.

![Climbing ropes secured with commits (anchors)](images/climbing.png)

Finally, `git push` uploads your local commits to GitHub, making them visible to collaborators (in RStudio, via the *Push* button in the Git tab). Essentially, `git commit` saves changes locally, and `git push` updates the remote repository.

When resuming a project after hours, days, or even months, `git pull` downloads any updates from GitHub (yours or your collaborators’), merging them with your local repository’s latest commit (in RStudio, via the *Pull* button).

![Git and GitHub workflow showing the working areas and commands used for transitions between them.](images/workflow_git_github.jpg)

In addition to these main buttons, in RStudio’s Git tab you’ll find the *Diff* button to review file changes since the last commit, and the branches (which we’ll cover later). Right-clicking lets you open modified files with *Open file*, or revert them to the last commit with *Revert* (⚠️ careful — this erases unsaved changes).

#### 📝**Exercise 1.4**

In the project you created in Exercise 1.2, save and upload your changes to GitHub (`git add` + `git commit` + `git push`).

💡 `git status`: shows your current branch and the changes made and added since the last commit.

In the remote GitHub repository, under the *Code* tab you’ll see your project content, including each commit made:
