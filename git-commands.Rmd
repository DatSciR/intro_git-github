...
author:
    - name: "Julen Astigarraga & Adriane Esquivel Muelbert"
date: "`r Sys.Date()`"
output: 
...
-   git commit: save the changes to the local repository
    ...
    -   `git diff \--staged`: show changes that are added but not commited yet
    ...
    -   `git commit \--amend`: change the message of you last commit
...
-   git reset: to undo a git commit or a git add
    ...
    -   `git reset \--hard HEAD\~1`: reset the records and all changes made to the working directory. `r emo::ji("danger")` *This is the most DANGEROUS option. Note that it deletes uncommitted changes and points your branch to the specified commit*
...
-   `git checkout main`: return to the main branch. `r emo::ji("danger")` *Note that if you change branches without committing first, all your changes will be overwritten*
