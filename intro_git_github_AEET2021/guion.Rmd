...
-   `git push --force`: se utiliza después de `git reset --hard` para sobreescribir la historia de la rama en la que estás trabajando. `r emo::ji("danger")r emo::ji("danger")` Esta opción es DOBLEMENTE PELIGROSA. No solo borras los cambios no comitidos si no que modificas la historia de un archivo. Si algún colaborador ya he descargado el archivo con su historia anterior, tendrá problemas cuando quiera integrar sus cambios de nuevo. ¡Utilizar sólo en casos extremos! (p. ej. publico sin querer mis claves de acceso a alguna base de datos).
