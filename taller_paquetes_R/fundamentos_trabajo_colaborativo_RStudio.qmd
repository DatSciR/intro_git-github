...
Esto se puede hacer en la [terminal](file:///C:/Users/julen/OneDrive/Escritorio/GitHub-col/intro_git-github/centra/dia2_colaboracion.html) y con el botón "*pull request*" en la página del proyecto en GitHub siguiendo los pasos que se explican a continuación:

Una vez que nos hemos cambiado de rama y hemos realizado los cambios que queríamos, subidlo a GitHub (`git add` + `git commit` + `git push`). En ese momento, en GitHub os aparecerá la opción de "Compare & pull request" y generáis el pull request ("*Create pull request*") añadiendo un mensaje para que sepáis lo que estáis uniendo. Os indicará si existen conflictos o no. Si no existen conflictos, podréis realizar el *pull request* sin problema, y si existen conflictos, hay que resolverlos manualmente.
