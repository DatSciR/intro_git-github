...
-   `knitr` empezó en el 2011 y R Markdown en 2014. Desarrollado principalmente por [Yihui Xie](https://yihui.org/)
...
-   Código (2 tipos):
    ...
    -   Code chunk (bloque de código): se escribe entre ```` ```{r}``` ````; `r` indica el lenguaje y se puede escribir manualmente o utilizar el atajo `Ctrl + Alt + I` / OS X `Cmd + Option + I` o utilizar el comando `Insert Code Chunk` en la barra de herramientas del editor o utilizar la paleta de comandos `Ctrl + Shift + P` / OS X `Cmd + Shift + P.`
...
Para compilar un archivo, al ejecutar el botón de `Render` o `Knit` en RStudio (atajo: `Ctrl + Shift + K` / OS X `Cmd + Shift + K`), llamará a Quarto/R Markdown render en un trabajo de fondo. Esto evitará que el renderizado abarrote la consola de R, y así es fácil de detener.
...
📝**Ejercicio:** algunos grupos generaran un documento R Markdown y otros un documento Quarto que esté compuesto por metadatos, código y texto (si alguien quiere añadir alguna tabla o algún otro elemento genial), y el objetivo será comparar la estructura y sintaxis de ambos documentos.
...
📝**Ejercicio:** convertiremos todos los archivos a `.qmd` y los renderizaremos a HTLM y word mediante la opción de renderización que más os guste.
...
📝**Ejercicio:** generaremos una plantilla y modificaremos los colores, tamaño de los títulos, etc. y lo añadiremos al YAML. Añadiremos alguna referencia en el documento y lo renderizaremos. Por último, subieremos este documento a nuestro repositorio de GitHub.
