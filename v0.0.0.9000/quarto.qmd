...
-   Código (2 tipos):
    ...
    -   Code chunk (trozo de código): se escribe entre ```` ```{r}``` ````; `r` indica el lenguaje y se puede escribir manualmente o utilizar el atajo `Ctrl + Alt + I` / OS X `Cmd + Option + I` o utilizar el comando `Insert Code Chunk` en la barra de herramientas del editor o utilizar la paleta de comandos `Cmd + Shift + P` / `Ctrl + Shift + P`
...
Al ejecutar el botón de `Render` o `knit` en RStudio, llamará a Quarto/R Markdown render en un trabajo de fondo. Esto evitará que el renderizado abarrote la consola de R, y así es fácil de detener.
